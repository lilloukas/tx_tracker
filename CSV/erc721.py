...
def main():
    ...
    with open("ERC721.csv", "w") as f:
        # Write the column headers
        f.write("Date,From,To,Hash,Gas,GasPrice,NFT Name, NFT ID\n")

        for address in addresses:
            # Set the parameters for the API request to get the transactions for the specified address
            params = {
                "module": "account",
                "action": "tokennfttx",  # Use the tokennftx action
                "address": address,
            ...
            if response.status_code == 200:
                ...
                for transaction in transactions:
                    ...
                    f.write(f"{date},{from_address},{to_address},{_hash},{gas},{gas_price},{nft_name},{nft_id}\n")
            else:
